...
public class Test {
	...
	private static double RAND = 0.1;
	...
	public static void main(String[] args) {
		...
		while ((System.nanoTime() - start)/1e9 < 20 * N) {
			...
			double vX = data[0][0]*Constants.MAX_STRAFE_SPEED + (Math.random()-0.5) * RAND;
			double vY = data[1][0]*Constants.MAX_FOR_SPEED + (Math.random()-0.5) * RAND;
			double vH = data[2][0]*Constants.MAX_ROT_SPEED + (Math.random()-0.5) * 3;
...
//				System.out.println((System.nanoTime() - start)/1e9 + "," + odometer.getXVelocity() + "," + odometer.getYVelocity() + "," + odometer.getAngularVelocity());
	...
	public static double[][] getInput() {
		...
		encX = data[0][0]*Constants.MAX_STRAFE_SPEED + (Math.random()-0.5) * RAND;
		encY = data[1][0]*Constants.MAX_FOR_SPEED + (Math.random()-0.5) * RAND;
		encH = data[2][0]*Constants.MAX_ROT_SPEED + (Math.random()-0.5) * 3;
		acX = data[0][0]*Constants.MAX_STRAFE_SPEED + (Math.random()-0.5) * RAND;
		acY = data[1][0]*Constants.MAX_FOR_SPEED + (Math.random()-0.5) * RAND;
		acH = data[2][0]*Constants.MAX_ROT_SPEED + (Math.random() - 0.5)/ 2;
	...
	public static double[][] getOutput() {
		...
		double vh = 18 / Constants.MAX_ROT_SPEED; 
		double h = vh * t;
		double gv = 2;
