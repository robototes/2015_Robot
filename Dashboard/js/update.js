...
function update() {
	...
	var MAX_VOLTAGE = 13;
	var VOLTAGE_WARNING_THRESHOLD = 10.0;
	var MAX_CURRENT = 160;
	var MAX_POWER = 1400;
	var MAX_TEMP = 80;
	...
	if (Systems.driveBase.voltage < VOLTAGE_WARNING_THRESHOLD && Systems.driveBase.voltage > 0) // if it's 0 then chances are its a comms problem, not battery
		RWS.alert(RWS.CAUTION, "Low Voltage", 10);
	else if (Systems.driveBase.voltage > MIN_VOLTAGE)
		RWS.reset("Low Voltage");
	
	if (Systems.driveBase.current > MAX_CURRENT)
		RWS.alert(RWS.WARNING, "High Current Draw", 10);
	else
		RWS.reset("High Current Draw");

	if (Systems.driveBase.temp > MAX_TEMP)
		RWS.alert(RWS.CAUTION, "High Temp", 5);
	else
		RWS.reset("High Temp");
...
RWS = (function() {
	...
	var alert = function(type, message, priority) {
		if (type <= lastType && priority <= lastPriority)
			return;
		...
		$(".rws").innerHTML = message.toUpperCase();
		$(".rws").style.backgroundColor = (type === 0) ? "#ec2" : "#e42";
		$(".rws").style.display = "block";
	...
	var clear = function() {
		lastType = -1;
		lastPriority = -1;
		$(".rws").style.display = "none";
	};

	var reset = function(message) {
		if (message === lastMessage && error) {
			clear();
			error = false;
		}
	};
