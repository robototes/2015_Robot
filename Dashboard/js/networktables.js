 ...
 * create() creates socket
 * send(BUFFER) sends buffer
 * connect(IP, PORT, CALLBACK) connects to previously created socket
 ...
 * isConnected(CALLBACK) passes true to callback if connection established
 */
function NetworkTable(name, IP, altLibrary) {
	...
	var chromeLibrary = (function() {
		...
		return {
			...
			connect: function(IP, PORT, CALLBACK) {
				...
				lib.connect(sID, IP, PORT, function() {
					CALLBACK();
				});
			},
			...
			isConnected: function(CALLBACK) {
				if (!sID)
					CALLBACK(false);
				return lib.getInfo(sID, function(i) {
					CALLBACK(i.connected);
				});
			}
		};
	})();
	...
	var checkConnection = function() {
		comm.isConnected(function(connected) {
			if (!connected)
